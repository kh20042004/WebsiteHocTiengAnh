...
public class AssignmentSubmissionApiController {
     ...
     * Extract user email from Authentication, then look up user ID
     * 
     * @return User ID (MongoDB ObjectId)
     * @throws com.english12smart.exception.ResourceNotFoundException if user not found
     */
    private String getCurrentUserId() {
        ...
        if (user == null) {
            throw new com.english12smart.exception.ResourceNotFoundException("Không tìm thấy người dùng với email: " + email);
        }
        log.debug("Current user: {} ({})", user.getEmail(), user.getId());
        return user.getId();
    }
