    ...
    public ResponseEntity<ApiResponse<?>> getSubmissions(
        ...
        try {
            ...
            if (teacherId == null || teacherId.isEmpty()) {
                log.warn("Unable to extract teacher ID from token");
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(ApiResponse.error("Không thể xác thực người dùng"));
            }
            
            log.debug("Teacher ID extracted: {}", teacherId);

            // ========== Call service ==========
            List<AssignmentSubmissionDTO.SubmissionListItemDTO> submissions =
                    submissionService.getSubmissionsByAssignment(assignmentId, teacherId);
            ...
            log.error("Unexpected error getting submissions", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Đã xảy ra lỗi. Vui lòng thử lại"));
        }
    }
