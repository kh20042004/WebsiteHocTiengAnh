...
public class AssignmentSubmissionApiController {
    ...
    public ResponseEntity<ApiResponse<?>> getSubmissions(
        ...
        try {
            ...
            log.debug("Token found, extracting teacher ID");
            String teacherId = null;
            try {
                teacherId = jwtTokenProvider.getUserIdFromToken(token);
            } catch (Exception e) {
                log.error("Error extracting teacher ID from token: {}", e.getMessage());
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(ApiResponse.error("Không thể xác thực người dùng"));
            }

            if (teacherId == null || teacherId.isEmpty()) {
                log.warn("Teacher ID is null or empty after extraction");
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(ApiResponse.error("Không thể xác thực người dùng"));
            }
