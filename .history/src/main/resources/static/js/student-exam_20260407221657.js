...
const ExamApp = (() => {
    ...
    // Anti-fraud manager
    let antiFraudManager = null;

    // ===== VIOLATION CALLBACK =====
    /**
     * Callback được gọi khi fraud detection vượt ngưỡng
     * Tự động nộp bài nếu vi phạm quá nghiêm trọng
     */
    function onFraudThresholdExceeded(violation) {
        console.error('🛑 FRAUD THRESHOLD EXCEEDED:', violation);
        
        // Hiển thị cảnh báo cho học sinh
        const warningMsg = violation.message || 'Bạn đã vi phạm quy định. Hệ thống sẽ tự động nộp bài.';
        ...
        // 🔴 TỰ ĐỘNG NỘP BÀI
        console.log('⏱️ Auto-submitting exam due to violation...');
        submitExam(true);
    }
    ...
    function init() {
        ...
        // ===== KHỞI TẠO HỆ THỐNG CHỐNG GIAN LẬN =====
        if (examData.submissionId && typeof ExamAntiFraudManager !== 'undefined') {
            antiFraudManager = new ExamAntiFraudManager(examData.submissionId, onFraudThresholdExceeded);
            antiFraudManager.enableStrictMode();
            console.log('✓ Hệ thống chống gian lận đã kích hoạt');
        }
    ...
    function startCountdown() {
        ...
        timerInterval = setInterval(() => {
            ...
            // Hết giờ: không tự động nộp, chỉ cảnh báo
            if (remainingSeconds <= 0) {
                clearInterval(timerInterval);
                const timerBlock = document.getElementById('timerBlock');
                if (timerBlock) {
                    timerBlock.classList.add('bg-red-100');
                    timerBlock.innerHTML = '<span class="text-red-600 font-bold">HẾT GIỜ</span>';
                }
                // Vô hiệu hóa nút "Câu sau" để ngăn thay đổi câu hỏi
                const nextBtn = document.getElementById('nextBtn');
                if (nextBtn) nextBtn.disabled = true;
                
                // Hiển thị cảnh báo nhưng KHÔNG tự động nộp - người dùng phải bấm nút "Nộp bài" chủ động
                // alert('⏰ Hết giờ! Hãy bấm nút "Nộp bài" để nộp bài làm của bạn.');
            }
        }, 1000);
    }
    ...
    function confirmSubmit() {
        ...
        let summaryHtml = `<div class="space-y-2 text-sm">
            <p class="font-semibold text-slate-900">Bạn có chắc chắn muốn nộp bài không?</p>
            <p class="text-slate-600">Đã trả lời: <strong class="text-violet-600">${answeredCount}/${totalCount}</strong> câu</p>`;
        
        if (unanswered > 0) {
            summaryHtml += `<p class="text-amber-600 font-medium">⚠️ Còn <strong>${unanswered}</strong> câu chưa trả lời</p>`;
        } else {
            summaryHtml += `<p class="text-emerald-600 font-medium">✓ Tất cả câu đều đã trả lời</p>`;
        }
        
        summaryHtml += `<p class="text-slate-500 text-xs">Lưu ý: Sau khi nộp, bạn không thể chỉnh sửa câu trả lời</p></div>`;
    ...
    async function submitExam(isAutoSubmit = false) {
        ...
        // 🔴 FLUSH pending fraud events trước khi nộp
        if (antiFraudManager) {
            console.log('📤 Flushing pending fraud events before submission...');
            await antiFraudManager.sendFraudEventBatch();
            // Chờ 1 giây để đảm bảo events đã gửi
