...
class ExamAntiFraudManager {
    constructor(submissionId, onThresholdExceeded = null) {
        this.submissionId = submissionId;
        this.fraudCount = {};
        this.isFullscreen = false;
        
        // Callback khi vượt ngưỡng vi phạm (auto-submit, etc.)
        ...
        this.tabChangeThreshold = 2; // Auto-submit nếu chuyển tab > 3 lần
    ...
    recordFraudEvent(fraudType, details) {
        ...
        this.checkViolationThresholds(fraudType);
    ...
    /**
     * Kiểm tra nếu vượt ngưỡng vi phạm
     * Nếu vượt ngưỡng, trigger callback (auto-submit, etc.)
     */
    checkViolationThresholds(fraudType) {
        // Nếu chuyển tab quá nhiều lần, tự động nộp bài
        if (fraudType === 'TAB_CHANGE' && this.fraudCount['TAB_CHANGE'] > this.tabChangeThreshold) {
            console.error(`🛑 TAB_CHANGE threshold exceeded (${this.fraudCount['TAB_CHANGE']} > ${this.tabChangeThreshold})!`);
            
            if (this.onThresholdExceeded) {
                this.onThresholdExceeded({
                    type: 'TAB_CHANGE_EXCEEDED',
                    count: this.fraudCount['TAB_CHANGE'],
                    threshold: this.tabChangeThreshold,
                    message: `Bạn đã chuyển tab quá ${this.fraudCount['TAB_CHANGE']} lần. Hệ thống sẽ tự động nộp bài.`
                });
            }
        }
    }
